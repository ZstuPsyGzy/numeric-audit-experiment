...
- 无 AI 基线固定在 AI 阶段之前；4 个 AI condition 采用一阶顺序平衡拉丁方；3 个 set size 采用 6 条平衡序列。
- AI condition 与 set size 顺序完整交叉为 `4 × 6 = 24` 个组；连续编号 `P001–P024` 构成一个完整周期，`P025` 开始下一周期。
- 材料种子对所有被试固定；被试编号只影响阶段和区组顺序。
- 当前不设置 trial 最长反应时间。
- 三个 set size（共 60 个正式 trial）完成后才结束一个 AI 条件；只在相邻 AI 条件之间显示自定进度休息页，不再每 20 个 trial 单独显示休息页。
- 300 个正式 trial 全部完成后，依次填写 Chinese BFI-10（10题、5点）和 Wang 等人的 AI Literacy Scale（12题、7点）；两份问卷均为必答，不插入数字任务过程。
...
`npm run check-experiment` 会逐一验证 300 个正式 trial 的条件计数、目标数、cue validity、系统事件和数字矩阵关系，并检查 `P001–P024` 的 24 组严格顺序平衡。

## 被试与设备

- 正式实验必须预先发放连续匿名编号，格式为 `P001、P002……`，并通过邮件或招募平台发送形如 `https://example.org/?sid=P001` 的专属链接。
- 为保持完整交叉平衡，计划样本量最好取 24 的整数倍；若不能，至少应按编号连续发放，不能跳号或任意自拟编号。
- 若某一编号中途退出，可使用该编号加 24 的同余编号补充同一顺序组，例如 `P017` 的替补使用 `P041`。
...
仓库已包含 GitHub Actions 校验：每次推送都会检查 300 个正式 trial、24 组顺序平衡，并构建一次 Docker 镜像。完整发布步骤见 [`GITHUB_RAILWAY_DEPLOY.md`](./GITHUB_RAILWAY_DEPLOY.md)。
