...
每名被试根据编号进入其中 1 个 AI 组合。每个组合包含 3 set size × 20 trials，共 60 AI trials。加上 60 个无 AI 基线 trial，每名被试总正式试次为 120。
...
深红和浅红提示均使用单元格内部的淡红底纹，不绘制红色边框，数字始终保持黑色。CR trial 中矩阵上不出现任何红色底纹，只显示中性的“AI 分析完成”。CR 不计入任何颜色 cue validity 的分母。
...
set size 使用 6 条完整平衡序列。正式编号使用前缀区分 AI 组：`A` = 90×90，`B` = 90×70，`C` = 70×90，`D` = 70×70。每个前缀内部的数字部分按 6 种 set size 顺序循环，例如 `A001–A006` 恰好覆盖 90×90 组的 6 种顺序。相同编号始终得到相同 AI 组和 set size 顺序。
...
- AI：`deep_outcome`、`light_outcome`、`cue_profile`、`cue_visual_style`、`cue_positions_overlap`、`system_event`、`system_correct`。
