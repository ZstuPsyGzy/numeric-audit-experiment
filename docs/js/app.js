...
const CONSENT_VERSION = "human-ai-consent-v2-2026-07-21";
...
function phaseIntro(phase) {
  if (phase === "baseline") {
    return {
      title: "独立审核阶段",
      content: `<div class="phase-intro"><p>本阶段不显示任何 AI 候选，用于测量你独立完成数字关系核查时的基线表现。</p><p>先完成 5 个练习 trial。练习正确率达到 80% 后进入正式实验。练习页面右上角可返回阅读指导语。</p></div>`
    };
  }
  return {
    title: "AI 辅助审核阶段",
    content: `<div class="phase-intro"><p>本阶段会显示深红和/或浅红候选。候选只是 AI 建议优先检查的位置，最终判断仍由你完成。</p><p>每个条件开始前会显示两类 cue 在该条件下的历史有效率。先完成 5 个练习 trial，达到 80% 后进入正式实验。练习页面右上角可返回阅读指导语。</p></div>`
  };
}
...
function buildTimeline(plan, assignment) {
  ...
  for (const phase of assignment.phase_order) {
    ...
    for (const block of blocks) {
      ...
      timeline.push({
        type: ExperimentScreenPlugin,
        title: "本组完成",
        content: `<div class="phase-intro"><p>已完成 ${block.length} 个 trial。可以短暂休息，准备好后继续。</p></div>`,
        button_label: "继续"
      });
    }
  }
  ...
  timeline.push({
    type: ExperimentScreenPlugin,
    title: "正式实验完成",
    content: "<div class=\"phase-intro\"><p>所有正式 trial 已完成。点击下方按钮退出全屏并生成实验数据文件。</p></div>",
