...
const AI_PROFILES = {
  "90_90": {
    ...
    target1: [
      ...repeat("deep_valid_light_invalid", 1),
      ...repeat("deep_invalid_light_valid", 1)
    ],
  ...
  "90_70": {
    ...
    target2: repeat("both_valid_split", 20)
  },
  "70_90": {
    target0: repeat("both_false_alarm", 2),
    target1: [
...
export function validateTrialPlan(plan = generateCanonicalPlan()) {
  ...
  for (const conditionKey of ["baseline", ...Object.keys(AI_CONDITIONS)]) {
    for (const setSize of SET_SIZES) {
      ...
      const condition = AI_CONDITIONS[conditionKey];
      if (row.correct_rejection !== 0 || row.miss !== 0) {
        errors.push(`${conditionKey}/S${setSize}: AI 阶段不应出现 CR 或 Miss`);
      }
      if (row.hit !== row.target_1 + row.target_2 || row.false_alarm !== row.target_0) {
        errors.push(`${conditionKey}/S${setSize}: Hit/FA 数量应分别等于有目标/无目标 trial 数`);
      }
      if (row.deep_displayed !== TRIALS_PER_CELL || row.light_displayed !== TRIALS_PER_CELL) {
        errors.push(`${conditionKey}/S${setSize}: 每个 AI trial 都应同时出现深红和浅红 cue`);
      }
      if (row.deep_valid !== condition.deep_validity * TRIALS_PER_CELL) {
        errors.push(`${conditionKey}/S${setSize}: 深红 cue 有效数错误`);
      }
      if (row.light_valid !== condition.light_validity * TRIALS_PER_CELL) {
